@extends('layouts.app')

@section('title', 'Detalles del Proyecto')

@section('header', 'Detalles del Proyecto')

@section('description', 'Información completa del proyecto seleccionado')

@section('content')
<div class="bg-white shadow-lg rounded-lg overflow-hidden">
    <div class="px-6 py-4 bg-gradient-to-r from-blue-600 to-blue-700">
        <h2 class="text-2xl font-bold text-white">{{ $proyecto->nombre }}</h2>
        <div class="mt-2">
            <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium
                @if($proyecto->estado === 'completado') bg-green-100 text-green-800
                @elseif($proyecto->estado === 'en_progreso') bg-yellow-100 text-yellow-800
                @elseif($proyecto->estado === 'iniciado') bg-blue-100 text-blue-800
                @else bg-red-100 text-red-800
                @endif">
                {{ $proyecto->estado_formateado }}
            </span>
        </div>
    </div>

    <div class="px-6 py-6">
        <dl class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                <dt class="text-sm font-medium text-gray-500 uppercase tracking-wide">ID del Proyecto</dt>
                <dd class="mt-1 text-lg text-gray-900">{{ $proyecto->id }}</dd>
            </div>

            <div>
                <dt class="text-sm font-medium text-gray-500 uppercase tracking-wide">Responsable</dt>
                <dd class="mt-1 text-lg text-gray-900">{{ $proyecto->responsable }}</dd>
            </div>

            <div>
                <dt class="text-sm font-medium text-gray-500 uppercase tracking-wide">Fecha de Inicio</dt>
                <dd class="mt-1 text-lg text-gray-900">{{ $proyecto->fecha_inicio_formateada }}</dd>
            </div>

            <div>
                <dt class="text-sm font-medium text-gray-500 uppercase tracking-wide">Monto</dt>
                <dd class="mt-1 text-lg font-semibold text-green-600">{{ $proyecto->monto_formateado }}</dd>
            </div>

            <div>
                <dt class="text-sm font-medium text-gray-500 uppercase tracking-wide">Fecha de Creación</dt>
                <dd class="mt-1 text-sm text-gray-900">
                    {{ $proyecto->fecha_creacion_formateada }}
                    <span class="text-gray-500">({{ $proyecto->fecha_creacion_relativa }})</span>
                </dd>
            </div>

            <div>
                <dt class="text-sm font-medium text-gray-500 uppercase tracking-wide">Última Actualización</dt>
                <dd class="mt-1 text-sm text-gray-900">
                    {{ $proyecto->fecha_actualizacion_formateada }}
                    <span class="text-gray-500">({{ $proyecto->fecha_actualizacion_relativa }})</span>
                </dd>
            </div>
        </dl>
    </div>

    <div class="px-6 py-4 bg-gray-50 border-t border-gray-200">
        <div class="flex flex-wrap gap-3">
            <a href="{{ route('proyectos.edit', $proyecto->id) }}" 
               class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-colors">
                <svg class="h-4 w-4 mr-2" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                </svg>
                Editar Proyecto
            </a>

            <a href="{{ route('proyectos.index') }}" 
               class="inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-colors">
                <svg class="h-4 w-4 mr-2" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18" />
                </svg>
                Volver a la Lista
            </a>

            <form method="POST" action="{{ route('proyectos.destroy', $proyecto->id) }}" class="inline-block">
                @csrf
                @method('DELETE')
                <button type="submit" 
                        onclick="return confirm('¿Estás seguro de que deseas eliminar este proyecto?')"
                        class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 transition-colors">
                    <svg class="h-4 w-4 mr-2" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                    </svg>
                    Eliminar Proyecto
                </button>
            </form>
        </div>
    </div>
</div>
@endsection 